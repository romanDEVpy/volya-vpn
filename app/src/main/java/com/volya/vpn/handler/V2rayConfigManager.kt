...
import com.volya.vpn.dto.ConfigResult
import com.volya.vpn.dto.ProfileItem
import com.volya.vpn.dto.RulesetItem
import com.volya.vpn.dto.V2rayConfig
import com.volya.vpn.dto.V2rayConfig.OutboundBean
import com.volya.vpn.dto.V2rayConfig.OutboundBean.OutSettingsBean
import com.volya.vpn.dto.V2rayConfig.OutboundBean.StreamSettingsBean
import com.volya.vpn.dto.V2rayConfig.RoutingBean.RulesBean
import com.volya.vpn.enums.EConfigType
import com.volya.vpn.enums.NetworkType
import com.volya.vpn.extension.isNotNullEmpty
import com.volya.vpn.extension.nullIfBlank
import com.volya.vpn.fmt.HttpFmt
import com.volya.vpn.fmt.Hysteria2Fmt
import com.volya.vpn.fmt.ShadowsocksFmt
import com.volya.vpn.fmt.SocksFmt
import com.volya.vpn.fmt.TrojanFmt
...
object V2rayConfigManager {
    ...
    private fun getV2rayGroupConfig(context: Context, guid: String, config: ProfileItem): ConfigResult {
        ...
        val volyaConfig = getV2rayMultipleConfig(context, config, configList) ?: return result

        result.status = true
        result.content = JsonUtil.toJsonPretty(volyaConfig) ?: ""
        result.guid = guid
    ...
    private fun getV2rayNormalConfig(context: Context, guid: String, config: ProfileItem): ConfigResult {
        ...
        val volyaConfig = initV2rayConfig(context) ?: return result
        volyaConfig.log.loglevel = MmkvManager.decodeSettingsString(AppConfig.PREF_LOGLEVEL) ?: "warning"
        volyaConfig.remarks = config.remarks

        getInbounds(volyaConfig)

        getOutbounds(volyaConfig, config) ?: return result
        getMoreOutbounds(volyaConfig, config.subscriptionId)

        getRouting(volyaConfig)

        getFakeDns(volyaConfig)

        getDns(volyaConfig)

        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_LOCAL_DNS_ENABLED) == true) {
            getCustomLocalDns(volyaConfig)
        }
        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_SPEED_ENABLED) != true) {
            volyaConfig.stats = null
            volyaConfig.policy = null
        }

        //Resolve and add to DNS Hosts
        if (MmkvManager.decodeSettingsString(AppConfig.PREF_OUTBOUND_DOMAIN_RESOLVE_METHOD, "1") == "1") {
            resolveOutboundDomainsToHosts(volyaConfig)
        }
    ...
    private fun getV2rayMultipleConfig(context: Context, config: ProfileItem, configList: List<ProfileItem>): V2rayConfig? {
        ...
        val volyaConfig = initV2rayConfig(context) ?: return null
        volyaConfig.log.loglevel = MmkvManager.decodeSettingsString(AppConfig.PREF_LOGLEVEL) ?: "warning"
        volyaConfig.remarks = config.remarks

        getInbounds(volyaConfig)

        volyaConfig.outbounds.removeAt(0)
        ...
        outboundsList.addAll(volyaConfig.outbounds)
        volyaConfig.outbounds = ArrayList(outboundsList)
    ...
    private fun getV2rayNormalConfig4Speedtest(context: Context, guid: String, config: ProfileItem): ConfigResult {
        ...
        volyaConfig.log.loglevel = MmkvManager.decodeSettingsString(AppConfig.PREF_LOGLEVEL) ?: "warning"
        volyaConfig.inbounds.clear()
        volyaConfig.routing.rules.clear()
        volyaConfig.dns = null
        volyaConfig.fakedns = null
        volyaConfig.stats = null
        volyaConfig.policy = null

        volyaConfig.outbounds.forEach { key ->
            key.mux = null
        }
     ...
     * the configuration from the "volya_config.json" asset file.
     *
    ...
    private fun initV2rayConfig(context: Context): V2rayConfig? {
        var assets = ""
        if (needTun()) {
            assets = initConfigCacheWithTun ?: Utils.readTextFromAssets(context, "volya_config_with_tun.json")
            if (TextUtils.isEmpty(assets)) {
                return null
            }
            initConfigCacheWithTun = assets
        } else {
            assets = initConfigCache ?: Utils.readTextFromAssets(context, "volya_config.json")
            if (TextUtils.isEmpty(assets)) {
                return null
            }
     ...
     *
     * @param volyaConfig The V2ray configuration object to be modified
     * @return true if inbound configuration was successful, false otherwise
     */
    private fun getInbounds(volyaConfig: V2rayConfig): Boolean {
        try {
            val socksPort = SettingsManager.getSocksPort()
            val socksUsername = SettingsManager.getSocksUsername()
            val socksPassword = SettingsManager.getSocksPassword()
            val inbound1 = volyaConfig.inbounds[0]
            ...
            if (!Utils.isXray()) {
                ...
                volyaConfig.inbounds.add(inbound2)
            }

            if (needTun()) {
                val inboundTun = volyaConfig.inbounds.firstOrNull { e -> e.tag == "tun" }
                inboundTun?.settings?.mtu = arrayListOf(SettingsManager.getVpnMtu())
     ...
     *
     * Adds FakeDNS configuration to volyaConfig if both local DNS and fake DNS are enabled.
     *
     * @param volyaConfig The V2ray configuration object to be modified
     */
    private fun getFakeDns(volyaConfig: V2rayConfig) {
        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_LOCAL_DNS_ENABLED) == true
            && MmkvManager.decodeSettingsBool(AppConfig.PREF_FAKE_DNS_ENABLED) == true
        ) {
            volyaConfig.fakedns = listOf(V2rayConfig.FakednsBean())
        }
    }
     ...
     *
     * Sets up the domain strategy, loads automatic Russia split-tunneling rules,
     * adds saved user rulesets, and appends a catch-all proxy rule.
     *
     * @param volyaConfig The V2ray configuration object to be modified
     * @return true if routing configuration was successful, false otherwise
     */
    private fun getRouting(volyaConfig: V2rayConfig): Boolean {
        try {

            volyaConfig.routing.domainStrategy =
                MmkvManager.decodeSettingsString(AppConfig.PREF_ROUTING_DOMAIN_STRATEGY)
                    ?: "AsIs"

            // Load default Russia split-tunneling rules (geoip:ru, geosite:category-ru → direct)
            loadDefaultRoutingRules(volyaConfig)

            // Load saved user rulesets (if any)
            val rulesetItems = MmkvManager.decodeRoutingRulesets()
            rulesetItems?.forEach { key ->
                getRoutingUserRule(key, volyaConfig)
            }

            // Catch-all: send all remaining traffic through proxy
            volyaConfig.routing.rules.add(
                RulesBean(
    ...
    /**
     * Loads default routing rules from the Russia split-tunneling preset.
     * Reads custom_routing_white_russia from assets and adds each rule.
     */
    private fun loadDefaultRoutingRules(volyaConfig: V2rayConfig) {
        try {
            val service = V2RayServiceManager.serviceControl?.get()?.getService() ?: return
            val jsonStr = Utils.readTextFromAssets(service, "custom_routing_white_russia")
            if (jsonStr.isNullOrEmpty()) return

            val rules = JsonUtil.fromJson(jsonStr, Array<RulesetItem>::class.java) ?: return
            rules.forEach { item ->
                if (item.enabled) {
                    val rule = JsonUtil.fromJson(JsonUtil.toJson(item), RulesBean::class.java) ?: return@forEach
                    volyaConfig.routing.rules.add(rule)
                }
            }
        } catch (e: Exception) {
            Log.e(AppConfig.TAG, "Failed to load default routing rules", e)
        }
    }
     ...
     * @param volyaConfig The V2ray configuration object to be modified
     */
    private fun getRoutingUserRule(item: RulesetItem?, volyaConfig: V2rayConfig) {
        try {
     ...
     */
    private fun getCustomLocalDns(volyaConfig: V2rayConfig): Boolean {
        try {
            ...
            // DNS outbound
            if (volyaConfig.outbounds.none { e -> e.protocol == "dns" && e.tag == "dns-out" }) {
                volyaConfig.outbounds.add(
                    OutboundBean(
                        protocol = "dns",
                        tag = "dns-out",
                        settings = null,
     ...
     */
    private fun getDns(volyaConfig: V2rayConfig): Boolean {
        try {
            ...
            // DNS dns
            volyaConfig.dns = V2rayConfig.DnsBean(
                servers = servers,
                hosts = hosts,
     ...
     */
    private fun getOutbounds(volyaConfig: V2rayConfig, config: ProfileItem): Boolean? {
        val outbound = convertProfile2Outbound(config) ?: return null
        val ret = updateOutboundWithGlobalSettings(outbound)
        if (!ret) return null

        if (volyaConfig.outbounds.isNotEmpty()) {
            volyaConfig.outbounds[0] = outbound
        } else {
            volyaConfig.outbounds.add(outbound)
        }
        return true
    }
     ...
     */
    private fun getMoreOutbounds(volyaConfig: V2rayConfig, subscriptionId: String): Boolean {
        //fragment proxy
        ...
        try {
            ...
            //current proxy
            val outbound = volyaConfig.outbounds[0]
            ...
            if (prevNode != null) {
                ...
                if (prevOutbound != null) {
                    ...
                    volyaConfig.outbounds.add(prevOutbound)
            ...
            if (nextNode != null) {
                ...
                if (nextOutbound != null) {
                    ...
                    volyaConfig.outbounds.add(0, nextOutbound)
     ...
     *
     * @param volyaConfig The V2ray configuration object to be modified with balancing settings
     * @param config The profile item containing policy group settings
     */
    private fun getBalance(volyaConfig: V2rayConfig, config: ProfileItem) {
        try {
            volyaConfig.routing.rules.forEach { rule ->
            ...
            when (config.policyGroupType) {
                ...
                "1" -> {
                    ...
                    volyaConfig.routing.balancers = listOf(balancer)
                    volyaConfig.burstObservatory = V2rayConfig.BurstObservatoryObject(
                ...
                "3" -> {
                    ...
                    volyaConfig.routing.balancers = listOf(balancer)
                    volyaConfig.observatory = V2rayConfig.ObservatoryObject(
                        subjectSelector = lstSelector,
            ...
            if (volyaConfig.routing.domainStrategy == "IPIfNonMatch") {
                volyaConfig.routing.rules.add(
                    RulesBean(
     ...
     * @param volyaConfig The V2ray configuration object to be modified
     */
    private fun resolveOutboundDomainsToHosts(volyaConfig: V2rayConfig) {
        val proxyOutboundList = volyaConfig.getAllProxyOutbound()
        val dns = volyaConfig.dns ?: return
